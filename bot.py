...
async def bot_makes_move(message):
	...
	cp_losing = engine_eval.split()[0] == 'cp' and int(engine_eval.split()[1]) <= 50*game.bot_skill - 2000
	cp_drawn = engine_eval.split()[0] == 'cp' and abs(int(engine_eval.split()[1])) <= 10
	# cp_losing threshold depends on skill, from -1000 at max skill to -3000 at min skill
...
async def on_message(message):
	...
	if message_text in ('--display', '--d'):
		...
		output = (f"Variant: **{game.variant}**"
				  ...
				  f"{chr(10) + 'Bot level: **' + game.startpos + '**' if game.player_is_playing(BOT_NAME) else ''}"
				  f"\nPosition: **{game.fen}**"
