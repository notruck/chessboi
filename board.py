...
class DrawBoard:
	def __init__(self, board_type, folder, flip_pieces, upside_down, fen, lastmove):
		self.fen = fen
		self.folder = folder
	...
	def get_piece_img(self, piece):
		...
		try:
			return PIL.Image.open(f'assets\\{self.folder}\\{colour}{piece}.png').convert('RGBA')
		except:
			return PIL.Image.open(f'assets\\fail.png').convert('RGBA')
	...
	def draw_board(self, stabilise_pocket=False):
		...
		# To make the game clip more stable, try to keep the pocket size at 2
		if stabilise_pocket and pocket_rows == 1:
			pocket_rows = 2
		...
		if self.board_type == "checkerboard": # chess style checkerboard
			...
			board = PIL.Image.open(f'assets\\{self.folder}\\board.png')
			if self.upside_down:
